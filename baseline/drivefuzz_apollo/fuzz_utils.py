...
def check_if_sp_avaliable(world:carla.World, sp:carla.Transform):
    blueprint_library = world.get_blueprint_library()
    player_bp = blueprint_library.filter('mercedes')[0]
    spawn_point = sp
    spawn_point.location.z += 1
    actor = world.try_spawn_actor(player_bp, spawn_point)
    if actor is None:
        print("[-] Failed spawning actor, try another location")
        return False
    else:
        actor.destroy()
        world.tick()
...
class TestScenario:
    ...
    def run_test(self, state):
        ...
        elif self.conf.agent_type == c.APOLLO:
            if error:
                shutil.copyfile(
                    os.path.join(self.conf.queue_dir, log_filename),
                    os.path.join(self.conf.error_dir, log_filename)
                )
            print('Copying video to error dir')
            shutil.copyfile(
                "/tmp/fuzzerdata/recorder.mp4",
                os.path.join(
                    self.conf.cam_dir,
                    log_filename.replace(".json", ".mp4")
                )
            )
        ...
        with open(os.path.join(self.conf.score_dir, log_filename), "w") as fp:
            json.dump(state.deductions, fp,indent=4)
