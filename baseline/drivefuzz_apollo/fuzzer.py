...
def init(conf:config.Config, args):
    ...
    conf.reload_carla_port = args.reload_port
    ...
    elif args.target.lower() == "apollo":
        conf.agent_type = c.APOLLO
    else:
...
def mutate_weather(test_scenario: fuzz_utils.TestScenario):
...
def mutate_weather_fixed(test_scenario: fuzz_utils.TestScenario):
...
def set_args():
    ...
    argparser.add_argument("-u", "--sim-host", default='172.17.0.1', type=str,
    ...
    argparser.add_argument("--reload-port", default=7691, type=int,
                           help="reloading carla server port")
    argparser.add_argument("-t", "--target", default="Apollo", type=str,
                           help="Target autonomous driving system (basic/behavior/autoware/Apollo)")
    ...
    argparser.add_argument("--town", default=10, type=int,
...
def main():
    ...
    queue = deque(conf.enqueue_seed_scenarios()) # initial scenario
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                client, _ = executor.connect(conf)
                time.sleep(10)
                try:
                    client.set_timeout(20.0)
                    world = client.load_world(town_map)
                except Exception as e:
                    print("[debug] reload failed")
                    sys.exit(-1)
            town = world.get_map()
            spawn_points = town.get_spawn_points()
            sp = random.choice(spawn_points)
            while not fuzz_utils.check_if_sp_avaliable(world, sp):
                sp = random.choice(spawn_points)
            sp_x = sp.location.x
            sp_y = sp.location.y
            sp_z = sp.location.z
            ...
            with open(os.path.join(conf.seed_dir, scene_name), "w") as fp:
                json.dump(seed_dict, fp,indent=4)
                ...
                while ret:
                    if actor_type == c.NULL:
                        ...
                        if nav_type == c.LINEAR:
                            ...
                            sp_idx = random.randint(
                                0, c.NUM_WAYPOINTS[town] - 1)
                            dp_idx = random.randint(
                                0, c.NUM_WAYPOINTS[town] - 1)
                            ret = test_scenario_m.add_actor(actor_type,
                            ...
                            if len(test_scenario_m.actors) == 0:
                                # add an actor
                                x = test_scenario_m.sp.location.x + \
                                    random.randint(-50, 50) / 10.0

                                y = test_scenario_m.sp.location.y + \
                                    random.randint(-50, 50) / 10.0
                                z = 1.5
                                ...
                                if direction == 0:
                                    ...
                                    test_scenario_m.actors[0]["maneuvers"][i] = [
                                        direction, speed, 0]
                                else:
                            ...
                            print("Maneuvers:",
                                  test_scenario_m.actors[0]["maneuvers"])
